...
def get_public_url(port, req=None):
    """
    CodeSandbox থেকে public URL বানাও।
    """
    # 1. CODESANDBOX_HOST env
...
def inject_port(file_path, port):
    ...
    try:
        ...
        new_run = 'app.run(host="0.0.0.0", port=int(__import__("os").environ.get("PORT", ' + str(port) + ')), debug=False)'
        pattern = r'app\.run\s*\([^)]*\)'
