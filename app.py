...
def get_public_url(port, req=None):
    ...
    """
    # 1. CODESANDBOX_HOST env (সবচেয়ে reliable)
    csb_host = os.environ.get("CODESANDBOX_HOST", "")
    if csb_host:
        if "$PORT" in csb_host:
            return f"https://{csb_host.replace('$PORT', str(port))}"
        else:
            return f"https://{port}-{csb_host}"

    # 2. Request host দিয়ে CodeSandbox detect
    if req:
        host = req.host
        if '.csb.app' in host or 'codesandbox.io' in host:
            parts = host.split('-', 1)
            if len(parts) == 2:
                return f"https://{port}-{parts[1]}"

    # 3. ngrok active tunnel check
    try:
...
if __name__ == "__main__":
    ...
    # 1. CODESANDBOX_HOST env variable (সবচেয়ে reliable)
    csb_host = os.environ.get("CODESANDBOX_HOST", "")
    if csb_host:
        if "$PORT" in csb_host:
            public_url = f"https://{csb_host.replace('$PORT', str(PANEL_PORT))}"
        else:
            public_url = f"https://{PANEL_PORT}-{csb_host}"

    # 2. CSB_SANDBOX_ID দিয়ে detect
    if not public_url:
        csb_id = os.environ.get("CSB_SANDBOX_ID", "")
        if csb_id:
            public_url = f"https://{csb_id}-{PANEL_PORT}.csb.app"

    # 3. ngrok try করো (pyngrok installed থাকলে)
    if not public_url:
        try:
